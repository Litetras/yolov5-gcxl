...
def detect(save_img=False):
    ...
    # Second-stage classifier用于对检测结果进行分类
    classify = False
    if classify:
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # Print time (inference + NMS)打印检测时间
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='data/images', help='source')  # default='data/images'代表直接检测文件夹里的图片, default改为0则默认用摄像头
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning) #更新所有模型
            for opt.weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:#权重文件
                detect()
